...
CRI_TOOLS_VER="v1.21.0"
CRI_TOOLS_URL=https://github.com/kubernetes-sigs/cri-tools/releases/download/$CRI_TOOLS_VER/crictl-${CRI_TOOLS_VER}-linux-${architecture}.tar.gz
CRI_TOOLS_TMP=$(mktemp -d)

curl -L ${CRI_TOOLS_URL} -o ${CRI_TOOLS_TMP}/crictl.tar.gz
tar zxvf ${CRI_TOOLS_TMP}/crictl.tar.gz -C ${CRI_TOOLS_TMP}
rm -f /opt/bin/crictl
mv ${CRI_TOOLS_TMP}/crictl /opt/bin/crictl-${CRI_TOOLS_VER}
ln -s /opt/bin/crictl-${CRI_TOOLS_VER} /opt/bin/crictl
